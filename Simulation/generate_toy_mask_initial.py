"""
draw_toy_gpu_mask.py
Masked source experiment with same-seed fairness across all samplers.
Outputs: tv_time_all_results_mask.csv
"""
...
def joint_to_index(joint_data):
    """Joint space is 8^3 (data tokens only, mask token excluded)."""
...
def run_sampler(sampler_name, posterior_model, path, vocab_size, x_init, N_step, delta):
    if sampler_name == "Tauleaping":
        ...
        return solver.sample(x_init=x_init, N=N_step, delta=delta, verbose=False, mask=True)
        ...
        solver = MixtureDiscreteRK2Solver(
            model=posterior_model, path=path, vocabulary_size=vocab_size)
        return solver.sample(x_init=x_init, N=N_step, delta=delta, theta=0.5, verbose=False, mask=True)
...
def main():
    ...
    joint_space_size = 8 ** 3  # data tokens only

    print("\n" + "=" * 80)
    print("Masked source experiment")
    print("=" * 80)
    ...
    for k in k_values:
        ...
        model_path = os.path.join("./ckpts", f"toy_mask_{k}_step200000", "ckpt.pth")
        ...
        for N_step in N_values:
            for rep in range(n_repeats):
                ...
                # Save RNG state — shared across all samplers and all N_step
                ...
                for sampler_name in samplers:
                    ...
                    x_init = (
                        torch.zeros(size=(n_samples_eval, dimension), device=device) + mask_token
                    ).long()
    ...
    # Save CSV
    csv_filename = "tv_time_all_results_mask_initial.csv"
