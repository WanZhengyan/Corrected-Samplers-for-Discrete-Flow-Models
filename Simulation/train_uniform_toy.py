...
def train(args, pretrained_model, info, start_step=0):
    ...
    if hasattr(torch, 'compile'):
        compiled_model = torch.compile(pretrained_model)
    else:
        compiled_model = pretrained_model
    
    log_every = 10000  # only sync CPU-GPU every N steps to maximize GPU utilization
    tqdm_step = tqdm.trange(start_step, n_steps)
    for step in tqdm_step:
        # Generate data directly on GPU — no CPU-GPU transfer needed
        x = generate_3k_discrete_data_gpu(n=batch_size, K=k, device=args.device)

        # x shape: (B, l)
        x_0 = torch.randint_like(x, high=vocab_size)
        t = torch.rand(x.shape[0], device=args.device) * (1 - delta)
        # sample probability path
        path_sample = path.sample(t=t, x_0=x_0, x_1=x)
        logits = compiled_model(x=path_sample.x_t, t=path_sample.t)
        loss = loss_fn(logits=logits, x_1=x, x_t=path_sample.x_t, t=path_sample.t)
        optimizer.zero_grad()
        loss.backward()    
        optimizer.step()

        # Only call loss.item() / wandb.log every log_every steps to avoid CPU-GPU sync stalls
        if step % log_every == 0:
            loss_val = loss.item()
            tqdm_step.set_description('Average Loss: {:5f}'.format(loss_val))
        # Update the checkpoint after each epoch of training.
        if step % 5000 == 4999 and args.save_model:
            save_dir = os.path.join("./ckpts", "toy_uniform_{}_step{}".format(k, step+1))
...
def main(args):
    for dir in ["./ckpts", "./toylogs"]:
        if not os.path.exists(dir):
            os.makedirs(dir)
    if not os.path.exists(os.path.join("./ckpts", "toy_uniform_{}".format(1e5))):
        os.makedirs(os.path.join("./ckpts", "toy_uniform_{}".format(1e5)))
    ...
    for k in [1, 2, 3, 4, 5]:
        ...
        checkpoint_path = os.path.join("./ckpts", f"toy_uniform_{k}_step100000", f"ckpt.pth")
        start_step = 0
