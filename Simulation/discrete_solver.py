...
class MixtureDiscreteRK2Solver(Solver):
    """Second-order Runge-Kutta solver for CTMC process defined by MixtureDiscreteProbPath.
    
    theta = 1/2: midpoint method
    theta = 1: Heun's method  
    theta = 2/3: Ralston's method
    
    NFE = 2 * N (two model evaluations per step)
    """
    def __init__(
        self,
        model: ModelWrapper,
    ...
    def sample(
        ...
        **model_extras,
        ...
        """
        Second-order Runge-Kutta sampling for CTMC.
        Args:
            x_init (Tensor): Initial state.
            t0 (float): Initial time.
            t1 (float): Final time.
            N (int): Number of segments in [0, 1-delta].
            delta (float): Early stopping threshold.
            theta (float): RK2 parameter in [0.5, 1]. 
                0.5 = midpoint, 1 = Heun, 2/3 = Ralston.
            opt_grid (bool): Whether to use optimized non-uniform grid. Defaults to True.
            mask (bool): Whether to handle mask tokens at the end.
            verbose (bool): Whether to print progress bars. Defaults to False.
            **model_extras: Extra model args.
        Returns:
            Tensor: Final state after RK2 sampling.
        ...
        if verbose:
            ...
            ctx = tqdm(total=N, desc=f"RK2 (theta={theta})")
        else:
            ctx = nullcontext()
        ...
        with ctx:
            for seg_idx in range(N):
                ...
                # ============================================================
                # Stage 1: Euler predictor step from t_left with step theta*h
                # ============================================================
                t_jump = torch.tensor([t_left], device=device)
                ...
                # Compute rate u_t(y, x_t) at t_left
                scheduler_output = self.path.scheduler(t=t_jump.repeat(batch_size))
                d_k_t = scheduler_output.d_alpha_t
                k_t = scheduler_output.alpha_t
                
                # Broadcast to (batch_size, dim, vocab_size)
                k_t_bc = k_t.view(batch_size, 1, 1)
                d_k_t_bc = d_k_t.view(batch_size, 1, 1)
                u_left = d_k_t_bc / (1 - k_t_bc) * p_1t
                
                # Set u_t(x_t, x_t) = 0
                delta_t = F.one_hot(x_t, num_classes=self.vocabulary_size)
                u_left = torch.where(delta_t.to(dtype=torch.bool), torch.zeros_like(u_left), u_left)
                
                # Euler step with step size theta*h to get x_mid
                intensity_pred = (u_left * theta * h).sum(dim=-1)
                mask_jump_pred = torch.rand(
                    size=x_t.shape, device=x_t.device
                ) < 1 - torch.exp(-intensity_pred)
                ...
                # ============================================================
                # Stage 2: Corrector step using weighted average of rates
                # ============================================================
                t_mid = t_left + theta * h
                t_mid_tensor = torch.tensor([t_mid], device=device)
                
                # Model prediction at midpoint
                p_1t_mid = self.model(x=x_mid, t=t_mid_tensor.repeat(batch_size), **model_extras)
                
                # Compute rate u_t(y, x_mid) at t_mid
                scheduler_output_mid = self.path.scheduler(t=t_mid_tensor.repeat(batch_size))
                d_k_t_mid = scheduler_output_mid.d_alpha_t
                k_t_mid = scheduler_output_mid.alpha_t
                
                # Broadcast to (batch_size, dim, vocab_size)
                k_t_mid_bc = k_t_mid.view(batch_size, 1, 1)
                d_k_t_mid_bc = d_k_t_mid.view(batch_size, 1, 1)
                u_mid = d_k_t_mid_bc / (1 - k_t_mid_bc) * p_1t_mid
                
                # Set u_t(x_mid, x_mid) = 0
                delta_t_mid = F.one_hot(x_mid, num_classes=self.vocabulary_size)
                u_mid = torch.where(delta_t_mid.to(dtype=torch.bool), torch.zeros_like(u_mid), u_mid)
                
                # Weighted average: final_u = (1 - 1/(2*theta)) * u_left + 1/(2*theta) * u_mid
                # Note: u_left is defined over vocab indices relative to x_t,
                # u_mid is defined over vocab indices relative to x_mid.
                # Since both are distributions over absolute vocab indices, we can combine them.
                # However, u_left should be re-zeroed for x_t's current position,
                # and u_mid is already zeroed for x_mid's current position.
                # For the final step, we sample from x_t (as in the generate_fast RK2).
                w1 = 1 - 1 / (2 * theta)
                w2 = 1 / (2 * theta)
                
                # Re-zero u_left at x_t positions (already done above)
                # Re-zero u_mid at x_t positions (for the combined rate from x_t)
                delta_t_for_combined = F.one_hot(x_t, num_classes=self.vocabulary_size)
                ...
                final_u = torch.clamp(final_u, min=0)  # ensure non-negative rates
                
                # Sample jump from x_t using the combined rate
                intensity = (final_u * h).sum(dim=-1)
                mask_jump = torch.rand(
                    size=x_t.shape, device=x_t.device
                ) < 1 - torch.exp(-intensity)
...
class MixtureDiscreteRK2TrapezoidSolver(Solver):
    """Theta-trapezoidal tau-leaping solver for CTMC process defined by MixtureDiscreteProbPath.
    
    Uses the theta-trapezoidal method where:
        alpha1(theta) = 1 / (2*theta)
        alpha2(theta) = ((1-theta)^2 + theta^2) / (2*theta)
        final_rate = alpha1 * u_mid - alpha2 * u_left
    
    The final jump is taken from x_mid (not x_t).
    ...
    def sample(
        ...
        **model_extras,
        ...
        """
        Theta-trapezoidal tau-leaping sampling for CTMC.
        Args:
            x_init (Tensor): Initial state.
            t0 (float): Initial time.
            t1 (float): Final time.
            N (int): Number of segments in [0, 1-delta].
            delta (float): Early stopping threshold.
            theta (float): Trapezoidal parameter in (0, 1].
            opt_grid (bool): Whether to use optimized non-uniform grid. Defaults to True.
            mask (bool): Whether to handle mask tokens at the end.
            verbose (bool): Whether to print progress bars. Defaults to False.
            **model_extras: Extra model args.
        Returns:
            Tensor: Final state after trapezoidal tau-leaping sampling.
        ...
        if verbose:
            ...
            ctx = tqdm(total=N, desc=f"RK2-Trapezoid (theta={theta})")
        else:
            ctx = nullcontext()
        ...
        with ctx:
            for seg_idx in range(N):
                ...
                # ============================================================
                # Stage 2: Corrector step using trapezoidal coefficients
                # Final jump is from x_mid (not x_t)
                # ============================================================
                t_mid = t_left + theta * h
                t_mid_tensor = torch.tensor([t_mid], device=device)
                ...
                # Re-zero u_left at x_mid positions (for the combined rate from x_mid)
                delta_t_for_combined = F.one_hot(x_mid, num_classes=self.vocabulary_size)
                u_left_rezeroed = torch.where(delta_t_for_combined.to(dtype=torch.bool), torch.zeros_like(u_left), u_left)
                
                # Trapezoidal combination: alpha1 * u_mid - alpha2 * u_left
                final_u = alpha1 * u_mid - alpha2 * u_left_rezeroed
                final_u = torch.clamp(final_u, min=0)  # clamp negative rates to 0
                
                # Sample jump from x_mid using the combined rate
                intensity = (final_u * h).sum(dim=-1)
                mask_jump = torch.rand(
